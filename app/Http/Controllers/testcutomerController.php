...
class CustomerController extends Controller
{
    ...
    public function checkout($staff_id,$order_id){
        ...
        $posid = DB::table('manage_devices')->where('staff_id',$staff_id)->value('posID') ?? ''; 
        $total_payment = DB::table('order_details')->where('id',$order_id)->value('total_payment');
        ...
        CURLOPT_URL => 'https://testca.valuepay.it/ecr2pos/posstatus',
        ...
        curl_close($curl);
        $pos_Stat = json_decode($response);
        if($pos_Stat->success==true){
           $orderdata=[
            'Action'=>$pos_Stat->Action,
            'TerminalId'=>$pos_Stat->TerminalId,
            'posID'=>$posid,
            'PosTime'=>$pos_Stat->PosTime,
            'PosStatus'=>$pos_Stat->PosStatus,
            'TextStatus'=>$pos_Stat->TextStatus,
            'ActionResult'=>$pos_Stat->ActionResult,
            'LicenseTerminalId'=>$pos_Stat->LicenseTerminalId,
            'LicenseCustomer'=>$pos_Stat->LicenseCustomer,
            'LicenseExpireDate'=>$pos_Stat->LicenseExpireDate,
            'status'=>'PENDING'

           ];
           $update = DB::table('order_details')->where('id',$order_id)->update($orderdata);
        ...
        return view('Customer.checkout',compact('staff_id','order_id','logo','data','posid','total_payment'));
   
    }

    public function pos_status(Request $request,$staff_id,$oid){
       $posid= $request->PosId;
        ...
        if($pos_Stat->success==true){
           ...
           $update = DB::table('order_details')->where('id',$oid)->update($orderdata);
           $oid_data = DB::table('order_details')->where('id',$oid)->first();
           return response()->json([
            ...
            return response()->json([
                'status'=>'0',
                'message'=>'POS Not Available in Services',
                'data'=>new stdClass()
               ]);
    ...
    public function payment_now(Request $request,$staff_id,$oid){
       $order_details = DB::table('order_details')->where('id',$oid)->where('staff_id',$staff_id)->first();
        if($order_details){
        $data=[
            "Version"=>"V.110",
            "PosId"=>$order_details->posID,
            "Action"=>"Payment", 
            "TerminalId"=>$order_details->TerminalId,
            "Amount"=>"0.01",
            "EcrId"=>"87654321",
            "ContractCode"=>"ValueSoft", 
            "PrintReceipt"=>"1"
        ];
        $curl = curl_init();

        curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://testca.valuepay.it/ecr2pos/payment',
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_ENCODING => '',
        CURLOPT_MAXREDIRS => 10,
        CURLOPT_TIMEOUT => 0,
        ...
        if($payment->success==true){
            ...
            if($payment->TransactionResult=="00"){

                DB::table('order_details')->where('id',$oid)->update(['status'=>'COMPLETED']);
                ...
                $payment=[
                    ...
                    'ProgressiveNumber'=>$payment->ProgressiveNumber,
                   ...
                   $update = DB::table('payment_transactions')->where('order_id',$oid)->first();
                   if(!$update){
                    DB::table('payment_transactions')->insert($payment);
                   }
                ...
                DB::table('order_details')->where('id',$oid)->update(['status'=>'CANCELED']);
            ...
            return response()->json([
                'status'=>'0',
                'message'=>'ECR Web Services Are Not Working'
            ]);
        ...
        return response()->json([
            'status'=>'0',
            'message'=>'OrderID does not exists'
        ]);
     }
    }
    ...
    public function posstatus(Request $request){
        ...
        // ];
            ...
            CURLOPT_CUSTOMREQUEST => 'POST',
            CURLOPT_POSTFIELDS =>'{
            "Version": "V.110","PosId": "3","Action": "PosStatus","TerminalId": "12345678"
            }',
    ...
    public function receipt($staff_id,$order_id)
    {
        ...
        // dd($order_item);
        $payment_transactions = DB::table('payment_transactions')->where('order_id',$order_id)->first();
        
        $Receipt = explode(" ",$payment_transactions->Receipt);
        $rec_data = array_filter($Receipt);
        // dd($rec_data);
        
       
        return view('Customer.reciept',compact('staff','order','order_item','rec_data'));
    }
