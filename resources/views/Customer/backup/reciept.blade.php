    ...
    <div id="bot">
        ...
        <div id="table">
            <table>
                ...
                <tr class="service">
                    <td class="tableitem">
                        <p class="itemtext">{{$item->menu_item_name}}</p>
                    </td>
                    <td class="tableitem">
                        <p class="itemtext">{{$item->count}}</p>
                    </td>
                    <td class="tableitem">
                        <p class="itemtext">€ {{number_format($item->total_price,2)}}</p>
                    </td>
                </tr>
                @endforeach
        ...
        <div id="legalcopy">
            ...
            <p style="text-align:center;">{{ $rec_data[9] }} {{ $rec_data[28] }} </p>
            <!-- <p style="text-align:center;"></p> -->
            <p style="text-align:center;">{{ $rec_data[46] }} {{ $rec_data[57] }}</p>
            <p style="text-align:center;">{{ $rec_data[58] }} {{ $rec_data[64] }}</p>
            <p style="text-align:center;">{{ $rec_data[65] }} </p>
            <!-- 92 => "Eserc."95 => "000102435130426A.I.I.C." -->
            <p style="text-align:center;">{{ $rec_data[92] }} {{ substr_replace($rec_data[95], "", -8) }} </p>
            <p style="text-align:center;"> {{ substr($rec_data[95],-8) }} {{ substr_replace($rec_data[100], "", -5) }}
            </p>
            <p style="text-align:center;"> {{ substr($rec_data[100],-4) }} {{ $rec_data[101] }} {{ $rec_data[103] }}
                {{ substr_replace($rec_data[104], "", -3) }} </p>

            <p style="text-align:center;">{{ substr($rec_data[104],-3) }} {{ $rec_data[105] }} {{ $rec_data[106] }}
                {{ $rec_data[107] }}.{{ $rec_data[108] }}</p>
            <p style="text-align:center;"> {{ $rec_data[114] }} {{ $rec_data[115] }}
                {{ $rec_data[116] }}.{{ $rec_data[117] }}</p>
            <p style="text-align:center;"> {{ $rec_data[125] }} {{ $rec_data[130] }} </p>
            <p style="text-align:center;"> {{ substr($rec_data[130],-4) }} {{ substr_replace($rec_data[146], "", -4) }}
                {{ substr($rec_data[146],-4) }} {{ substr_replace($rec_data[152], "", -4) }} </p>
            <p style="text-align:center;"> {{ substr($rec_data[152],-4) }} {{ $rec_data[161] }}
                {{ substr_replace($rec_data[162], "", -3) }} </p>

            <p style="text-align:center;"> {{ substr($rec_data[162],-3) }} {{ $rec_data[163] }}
                {{ $rec_data[165] }}{{ $rec_data[166] }} {{ $rec_data[168] }} {{ $rec_data[169] }}</p>

            <p style="text-align:center;">{{ substr($rec_data[169],-4) }} {{ $rec_data[170] }}
                &nbsp;{{ $rec_data[175] }} {{ substr_replace($rec_data[176], "", -3) }} </p>

            <p style="text-align:center;">{{ substr($rec_data[176],-3) }}
                {{ $rec_data[187] }}{{ substr_replace($rec_data[191], "", -3) }} </p>

            <p style="text-align:center;">{{ substr($rec_data[191],-3) }} {{ substr_replace($rec_data[198], "", -3) }}
            </p>
            <p style="text-align:center;">{{ substr($rec_data[198],-3) }} {{ $rec_data[199]}}</p>
            <p style="text-align:center;"> {{ $rec_data[238] }}&nbsp;{{ $rec_data[239] }} &nbsp; {{$rec_data[248]}}</p>
            <p style="text-align:center;">{{ $rec_data[273] }}&nbsp;{{ $rec_data[274] }}&nbsp;{{ $rec_data[275] }}
                &nbsp; {{ $rec_data[276] }}</p>
            <p style="text-align:center;">{{ $rec_data[302] }} </p>
            <p style="text-align:center;">{{ $rec_data[327] }}&nbsp;{{ $rec_data[328] }}&nbsp;{{ $rec_data[356] }} </p>
            <p style="text-align:center;">{{ $rec_data[357] }}&nbsp;{{ $rec_data[358] }} </p>
            <p style="text-align:center;">{{ $rec_data[360] }}&nbsp;{{ $rec_data[361] }} </p>
